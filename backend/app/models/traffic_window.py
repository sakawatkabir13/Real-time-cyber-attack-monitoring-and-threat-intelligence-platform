...
class TrafficWindow(Base):
    ...
    feature_schema = Column(Integer, nullable=False, default=3, server_default="2")
    request_time_coverage = Column(Float)
    peak_second_requests = Column(Float)
    burst_ratio = Column(Float)
    rate_change_ratio = Column(Float)
    previous_window_present = Column(Float)
    failed_auth_ratio = Column(Float)
    max_path_unique_ips = Column(Float)
